...
/////////////////////////////////////////////////////////////////////////////////////
			...
			if(flag==1 && amount>=35){//UBLOX M8 rinex3.02
				...
				while(return_flag != satn){
					...
					if(type_sat==0 || type_sat==3 || type_sat==1 || type_sat==6 || type_sat==2){
						...
						if(type_sat==2 && sv_number==31)
							sv_number = 0;//強制的にE31番は40として利用しない
 ...
 //|             |              (may suffer from increased noise) 
						...
						for(i=16*4;i<=16*4+15;i++)
							buff2[i-16*4]=buff[i];
						buff2[15]='\0';
						Pr2[rcvn][sv_number+sv_offset]=atof(buff2);

						for(i=16*5;i<=16*5+15;i++)
							buff2[i-16*5]=buff[i];
						buff2[15]='\0';
						Cp2[rcvn][sv_number+sv_offset]=atof(buff2);
						LLI2[rcvn][sv_number+sv_offset] = atoi(&buff2[14]);

						for(i=16*6;i<=16*6+15;i++)
							buff2[i-16*6]=buff[i];
						buff2[15]='\0';
						Dp2[rcvn][sv_number+sv_offset]=atof(buff2);

						for(i=16*7;i<=16*7+15;i++)
							buff2[i-16*7]=buff[i];
						buff2[15]='\0';
						Cn2[rcvn][sv_number+sv_offset]=atof(buff2);

...
void calc_rtk_GQB_F9P(int rcvn)
{
	int i,j,k,ndim;
	int prn;
...
//	w_inverse_float(w,nsv,max_prn,flag);
//	w_inverse_float2(w,SATn[rcvn]-2,flag,nsvg,nsvb);
	...
	for(i=0;i<SATn[rcvn];i++){
		...
		if(prn != max_prn_bei && prn>=71){
			...
			dd_carrier_l2[prn] = cs/B2*((Cp2[0][prn] - Cp2[1][prn])
				-(Cp2[0][max_prn_bei] - Cp2[1][max_prn_bei]));

			dd_code[prn] = (Pr1[0][prn] - Pr1[1][prn])
				-(Pr1[0][max_prn_bei] - Pr1[1][max_prn_bei]);
			dd_code_l2[prn] = (Pr2[0][prn] - Pr2[1][prn])
				-(Pr2[0][max_prn_bei] - Pr2[1][max_prn_bei]);

			n1[prn] = dd_carrier[prn]*B1/cs - dd_code[prn]*B1/cs;
			n2[prn] = dd_carrier_l2[prn]*B2/cs - dd_code_l2[prn]*B2/cs;

			OmomiPB[j] = Code_noise+Code_noise/sin(Elevation[rcvn][prn]*pi/180.0);
			OmomiCB[j] = Carrier_noise+Carrier_noise/sin(Elevation[rcvn][prn]*pi/180.0);
...
//			OmomiC[j] = Carrier_noise*1;
	...
	flag=2;
	w_inverse_float2(w,SATn[rcvn]-2,flag,nsvg,nsvb,nsvc,nsvr);


	j=0;//gps l1 bei b1 gpsl2 bei b2(carrier) + gps l1 bei b1 gpsl2 bei b2(code)
	for(i=0;i<SATn[rcvn];i++){
		prn = SVn[rcvn][i];
		if(prn != max_prn_gps && prn<=39)
			y[j++] = dd_carrier[prn];
...
////////////////////////////////////////////////////////////////////////////////////////////
	...
	for(iter=1;iter<=4;iter++){
		//GçsóÒÇÃê∂ê¨Å@è„Ç©ÇÁL1î¿ëóîg(GPS+BEI), L2î¿ëóîg(GPS+BEI), L1ÉRÅ[Éh(GPS+BEI), L2ÉRÅ[Éh(GPS+BEI)
		for(i=0;i<nsv*2+3;i++){
			for(j=0;j<nsv*4;j++){
				if(j<nsvg)
					...
					g[i+j*(nsv*2+3)]=-1.0*(lrkn[i][j%nsv]-lrln[i]);
				else if(j>=2*nsvg+2*nsvb && j<3*nsvg+2*nsvb)
					g[i+j*(nsv*2+3)]=-1.0*(lrkn[i][j%nsv]-lrln[i]);
				else if(j>=3*nsvg+3*nsvb && j<4*nsvg+3*nsvb)
					g[i+j*(nsv*2+3)]=-1.0*(lrkn[i][j%nsv]-lrln[i]);
				else
					g[i+j*(nsv*2+3)]=-1.0*(lrkn[i][j%nsv]-lrlnb[i]);
			}
		}
...
//		minver4(gtwg,eps,nsv*2+3);
		...
		//DopplerèÓïÒÇóòópÇ∑ÇÈèÍçáÇÃèÍçáÇÌÇØ
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	for(i=nsv*2;i<nsv*4;i++)//écç∑ç≈è¨ÇÃåÛï‚ÅiÉAÉìÉrÉMÉÖÉCÉeÉBÇªÇÃÇ‡ÇÃÅjÇÃÇ›écÇ∑
		F[i]=0;

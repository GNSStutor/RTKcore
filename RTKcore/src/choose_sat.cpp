...
//				Reject_cn[rcvn][prn]==1 ||
				...
				fabs(Pr2[rcvn][prn]) <= 1.0 ||//擬似距離の有無
				fabs(Cp2[rcvn][prn]) <= 1.0 ||//搬送波位相の有無
				LLI[rcvn][prn] >= 2 ||//Loss of lock indicator(スリップやハーフサイクルチェック)
				LLI2[rcvn][prn] >= 2 ||//Loss of lock indicator(スリップやハーフサイクルチェック)
	...
	if(rcvn == 0){
		for(i=0;i<SATn[rcvn];i++){
			...
			if(
				...
				fabs(Pr2[rcvn][prn]) <= 1.0 ||//擬似距離の有無
				fabs(Cp2[rcvn][prn]) <= 1.0 ||//搬送波の有無
				LLI[rcvn][prn] >= 2 ||
				LLI2[rcvn][prn] >= 2 ||
				prn > 1000)//Loss of lock indicator
				i=i;
			else{
				stock_sat[sat] = SVn[rcvn][i];
				sat++;

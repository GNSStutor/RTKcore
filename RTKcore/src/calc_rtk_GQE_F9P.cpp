...
void calc_rtk_GQE_F9P(int rcvn)
{
	int i,j,k,ndim;
	int prn;
	...
	for(i=0;i<SATn[rcvn];i++){
		...
		if(prn != max_prn_bei && prn>=41 && prn<=70){
			//î¿ëóîgÇÃïÑçÜÇ…íçà”
			dd_carrier[prn] = cs/E1*((Cp1[0][prn] - Cp1[1][prn])
				-(Cp1[0][max_prn_bei] - Cp1[1][max_prn_bei]));
			dd_carrier_l2[prn] = cs/E5B*((Cp2[0][prn] - Cp2[1][prn])
			...
			n1[prn] = dd_carrier[prn]*E1/cs - dd_code[prn]*E1/cs;
			n2[prn] = dd_carrier_l2[prn]*E5B/cs - dd_code_l2[prn]*E5B/cs;
	...
	for(iter=1;iter<=3;iter++){
		...
		for(i=0;i<=2;i++){
			...
			for(j=nsvg;j<nsvg+nsvb;j++){
				g[i+j*3]=-1.0*(lrkn[i][j]-lrlnb[i])*E1/cs;
			}
		}

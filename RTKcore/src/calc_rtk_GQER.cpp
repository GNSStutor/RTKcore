...
void calc_rtk_GQER(int rcvn)
{
	...
	for(i=0;i<SATn[rcvn];i++){
		...
		if(prn != max_prn_bei && prn>=101){
			...
			OmomiPB[j] = Code_noise*1.0+Code_noise*1.0/sin(Elevation[rcvn][prn]*pi/180.0);
			OmomiCB[j] = Carrier_noise*1.0+Carrier_noise*1.0/sin(Elevation[rcvn][prn]*pi/180.0);

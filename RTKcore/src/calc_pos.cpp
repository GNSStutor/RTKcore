...
//		while(iter<=6)
	...
	for(i=0;i<SATn[rcvn];i++){
		...
		Pr2[rcvn][j]=Pr2[rcvn][j]-Clock[rcvn];

		//ここではRTK用に衛星時計側のクロック変動の影響を補正する
		//基準局と移動局で補正データの遅延（AGE）があるので、それを考慮
		if(rcvn==0 && Partial_flag == 0){
			if(j<=100){
				Pr1[1][j] = Pr1ref[j] - cs*(DGPSTIME-GPSTIME)*Ephe.af1[j];
				Pr2[1][j] = Pr2ref[j] - cs*(DGPSTIME-GPSTIME)*Ephe.af1[j];
			}
			if(j>=101 && j<=130){
				Pr1[1][j] = Pr1ref[j] - cs*(DGPSTIME-GPSTIME)*Ephe.GammaN[j];
				Pr2[1][j] = Pr2ref[j] - cs*(DGPSTIME-GPSTIME)*Ephe.GammaN[j];
			}

			if(j>=1 && j<=39){
				Cp1[1][j] = Cp1ref[j] - cs*(DGPSTIME-GPSTIME)*(Ephe.af1[j]*F1/cs);
				Cp2[1][j] = Cp2ref[j] - cs*(DGPSTIME-GPSTIME)*(Ephe.af1[j]*F2/cs);
			}
			if(j>=41 && j<=70){
				Cp1[1][j] = Cp1ref[j] - cs*(DGPSTIME-GPSTIME)*(Ephe.af1[j]*E1/cs);
				Cp2[1][j] = Cp2ref[j] - cs*(DGPSTIME-GPSTIME)*(Ephe.af1[j]*E5B/cs);
			}
			if(j>=71 && j<=100){
				Cp1[1][j] = Cp1ref[j] - cs*(DGPSTIME-GPSTIME)*(Ephe.af1[j]*B1/cs);
				Cp2[1][j] = Cp2ref[j] - cs*(DGPSTIME-GPSTIME)*(Ephe.af1[j]*B2/cs);
			}
			if(j>=101 && j<=130){
				Cp1[1][j] = Cp1ref[j] - cs*(DGPSTIME-GPSTIME)*(Ephe.GammaN[j]*GF1[j]/cs);
				Cp2[1][j] = Cp2ref[j] - cs*(DGPSTIME-GPSTIME)*(Ephe.GammaN[j]*GF2[j]/cs);
			}
		}

		//if(j<=70 && rcvn==1 && POS==1)
		//	Cp1[rcvn][j]=Cp1[rcvn][j]-Clock[rcvn]/(cs/F1);
		//if(j<=70 && rcvn==0 && POS==2)
		//	Cp1[rcvn][j]=Cp1[rcvn][j]-Clock[rcvn]/(cs/F1);

		//if(j>=71 && j<=100 && rcvn==1 && POS==1)
		//	Cp1[rcvn][j]=Cp1[rcvn][j]-Clock[rcvn]/(cs/B1);
		//if(j>=71 && j<=100 && rcvn==0 && POS==2)
		//	Cp1[rcvn][j]=Cp1[rcvn][j]-Clock[rcvn]/(cs/B1);

		if(j>=101 && j<=130 && rcvn==1 && POS==1){
			Cp1[rcvn][j]=Cp1[rcvn][j]-Clock[rcvn]/(cs/GF1[j]);
			Cp2[rcvn][j]=Cp2[rcvn][j]-Clock[rcvn]/(cs/GF2[j]);
		}
		if(j>=101 && j<=130 && rcvn==0 && POS==2){
			Cp1[rcvn][j]=Cp1[rcvn][j]-Clock[rcvn]/(cs/GF1[j]);
			Cp2[rcvn][j]=Cp2[rcvn][j]-Clock[rcvn]/(cs/GF2[j]);
		}
	}

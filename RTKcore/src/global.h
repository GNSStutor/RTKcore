...
void calc_rtk_GQ_F9P(int);
void calc_rtk_GQB_F9P(int);
void calc_rtk_GQE_F9P(int);
void calc_rtk_GQEB_F9P(int);
void calc_rtk_GQEBR_F9P(int);
...
int No_rtk,Max_prn;
int LLI[RCVN][PRN],LLI2[RCVN][PRN];
...
int RTK,Doppler;
int Reject_cn[RCVN][PRN];
int Loose_flag,Partial_flag;

double Elevation_mask1;
double Threshold_cn,HDOP,VDOP,PDOP,GDOP,TDOP,Ratio_limit;
double Pr1[RCVN][PRN],SPr1[RCVN][PRN],OPr1[RCVN][PRN];//擬似距離
double Cp1[RCVN][PRN];//搬送波位相
double Cn1[RCVN][PRN],Dp1[RCVN][PRN];//信号強度、ドップラ周波数
double Pr2[RCVN][PRN],Cp2[RCVN][PRN],Dp2[RCVN][PRN],Cn2[RCVN][PRN];
double SVx[RCVN][PRN],SVy[RCVN][PRN],SVz[RCVN][PRN];//衛星位置
double SVx_next[RCVN][PRN],SVy_next[RCVN][PRN],SVz_next[RCVN][PRN];//衛星位置
double SVx_prev[RCVN][PRN],SVy_prev[RCVN][PRN],SVz_prev[RCVN][PRN];//衛星位置
...
double LS_update[3];
double OmomiP[PRN],OmomiC[PRN],OmomiPB[PRN],OmomiCB[PRN],OmomiPR[PRN],OmomiCR[PRN],OmomiPRR[PRN],OmomiCRR[PRN];
double GF1[PRN],GF2[PRN];
double Pr1ref[PRN],Cp1ref[PRN],Pr2ref[PRN],Cp2ref[PRN];
double User_lat,User_lon,User_hgt,User_pos[3];
double LeapSecond;
double Correction[PRN];
double Vel[3],Vel_lat,Vel_lon,Vel_hgt,Pre_vel[3];
double DGPS_lat,DGPS_lon,DGPS_hgt,Loose_pos[3],Loose_lat,Loose_lon,Loose_hgt;
double RTK_lat,RTK_lon,RTK_hgt;
double Fix_pos[3],Code_omomi;

...
void calc_rtk_GQEB_F9P(int rcvn)
{
	...
	j=0;
	//â¬éãâqêØÇÃ2èdà ëäç∑ÇÃéZèo Galileo
	for(i=0;i<SATn[rcvn];i++){
		prn = SVn[rcvn][i];
...
//			OmomiCR[j] = Carrier_noise*1;
	...
	j=0;
	//â¬éãâqêØÇÃ2èdà ëäç∑ÇÃéZèo BEIDOU
	for(i=0;i<SATn[rcvn];i++){
		prn = SVn[rcvn][i];
...
////////////////////////////////////////////////////////////////////////////////////////////
	...
	for(iter=1;iter<=4;iter++){
		//GçsóÒÇÃê∂ê¨Å@è„Ç©ÇÁL1î¿ëóîg(GPS+BEI+GAL), L2î¿ëóîg(GPS+BEI+GAL), L1ÉRÅ[Éh(GPS+BEI+GAL), L2ÉRÅ[Éh(GPS+BEI+GAL)
		for(i=0;i<nsv*2+3;i++){
			for(j=0;j<nsv*4;j++){
				if(j<nsvg)
